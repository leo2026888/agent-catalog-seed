...
当前可用：**22 条公开目录记录**，其中1个为本项目公开 Skill，21条为第三方来源记录。2026-09-07新增Notion官方托管MCP服务，并把context7与Chrome DevTools MCP从公开元数据初筛推进到固定提交README和许可证深化复核；其余14条批量候选仍为元数据初筛。完整运行评测、自动安装与团队持续服务仍在验证。

**当前版本：v0.4.0-seed 每日证据深化。** 公开仓库：[leo2026888/agent-catalog-seed](https://github.com/leo2026888/agent-catalog-seed)。 本次新写的代码、Schema、合成示例与公开适配版 Skill 指令已声明 MIT 许可；文章与方法文档采用 CC BY 4.0，署名为 Leo and contributors，披露 AI 辅助创作与整理，具体范围见 [许可与署名](LICENSING.md)。GitHub 仓库已创建并核验为 public。私密漏洞报告已启用，入口及尚未完成的报告/通知测试见 [安全政策](SECURITY.md)。没有交易结算或自动运行评测平台，没有经过独立安全认证，没有确认的平台接入伙伴，也没有已经复核计入的真实用户评分。公开资料评分与任务效果分开显示；第三方公开来源观察单独记录在 `evidence/`，不改写为已运行或已通过独立评测的条目。
...
| 查看Notion MCP及两条深化复核 | [2026-09-07每日批次](evidence/daily-catalog-batch-2026-09-07.json)；托管服务版本与本地参考实现版本分开记录 |
