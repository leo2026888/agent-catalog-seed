...
此模块让智能体用三个操作查询本项目的五条公开记录。已用本地测试客户端完成 MCP stdio 初始化握手、工具发现及三种工具调用。**WorkBuddy、豆包、千问、Codex 的实际导入与调用均未验证。** 本地协议检查不等于候选插件运行评测，也不等于完整 MCP 一致性认证。v0.2.0-seed已发布源代码，发布不代表客户端接入完成。
...
`null` 表示未知，不等于零分或已通过。五条记录的候选实际运行、独立效果复核与安全认证均没有完成。根仓库 `entries/` 的合成示例不参与此真实目录检索。

检索采用有限的中文／英文关键词匹配，并按匹配词数量排序；它没有语义模型、向量检索或效果评分。重叠任务可能返回多个候选，通用词也可能产生误匹配。无匹配仅描述这五条记录的覆盖范围。搜索词最多512字符，工具单条输入消息最多64 KiB。
