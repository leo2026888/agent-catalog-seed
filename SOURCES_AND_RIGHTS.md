...
| `evidence/` 的23条目录记录和说明 | 依据公开仓库元数据、维护与社区字段、官方服务文档及首轮来源记录重新编写，AI辅助整理；精确来源URL和观察边界见各批次 | 没有复制第三方Skill正文、脚本、图片或安装包；第三方记录只保存事实字段、原创简介和官方链接 | 本项目新增摘要与编排采用CC BY 4.0；各仓库许可元数据仅描述上游声明，不由本项目重新授权第三方内容；Notion本地参考实现MIT不自动扩展到托管服务；Figma服务受其使用条款约束，本站未获服务或内容再分发权 |
